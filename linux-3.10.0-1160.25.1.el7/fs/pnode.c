...
int max_mnt_share_count;
int max_mnt_slave_count;
int max_mnt_slave_list_count;

static int do_make_slave(struct mount *mnt)
{
	struct mount *master, *slave_mnt;
	int mnt_share_count = 0;
	int mnt_slave_count = 0;
	int mnt_slave_list_count = 0;
	...
	if (list_empty(&mnt->mnt_share)) {
		...
		if (!master) {
			...
			while (!list_empty(p)) {
				// 从链表 p 中获取第一个条目，并将其赋值给 slave_mnt
				slave_mnt = list_first_entry(p, struct mount,
							     mnt_slave);
				// 将 slave_mnt 从链表中删除，并重新初始化该链表节点
	...
	list_for_each_entry (slave_mnt, &mnt->mnt_slave_list, mnt_slave) {
		...
		mnt_slave_list_count++;
	}

	if (mnt_share_count > max_mnt_share_count)
		max_mnt_share_count = mnt_share_count;
	if (mnt_slave_list_count > max_mnt_slave_list_count)
		max_mnt_slave_list_count = mnt_slave_list_count;
	if (max_mnt_slave_count > mnt_slave_count)
		max_mnt_slave_count = mnt_slave_count;
...
static int mnt_list_proc_show(struct seq_file *m, void *v)
{
	seq_printf(
		...
		max_mnt_share_count, max_mnt_slave_count,
		max_mnt_slave_list_count);
	return 0;
}
