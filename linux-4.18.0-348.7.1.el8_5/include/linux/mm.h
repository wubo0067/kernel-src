...
struct vm_operations_struct {
	 ...
	 */
	RH_KABI_USE(1, int (*mprotect)(struct vm_area_struct *vma,
...
extern struct vm_area_struct *
	vma_merge(struct mm_struct *, struct vm_area_struct *prev,
		  unsigned long addr, unsigned long end, unsigned long vm_flags,
		  struct anon_vma *, struct file *, pgoff_t, struct mempolicy *,
...
static inline unsigned long vm_start_gap(struct vm_area_struct *vma)
{
	...
	if (vma->vm_flags & VM_GROWSDOWN) {
		// 如果是个堆栈区域，并且是向下增长的，可以理解为这是个栈区域
		// 这种情况通常出现在操作系统将栈区域从高地址向低地址生长的情况下。
		// 在这种情况下，栈指针会不断向较低地址移动，而栈区域的起始地址会不断向较高地址移动。
		// 为了避免栈溢出等问题的发生，操作系统通常会在栈区域的开始位置留出一段空闲区域，称为“栈空洞”，
		// 这个空洞的大小通常是一个常量，称为“stack_guard_gap”。
		vm_start -= stack_guard_gap;
		if (vm_start > vma->vm_start)
			// 如果比他大，只有一种可能，就是vm_start是小于stack_guard_gap，对于无符号的数就会变成很大的数
			vm_start = 0;
	}
	return vm_start;
}
