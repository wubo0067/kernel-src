...
#define xa_for_each_range(xa, index, entry, start, last)                       \
	for (index = start, entry = xa_find(xa, &index, last, XA_PRESENT);     \
...
#define __XA_STATE(array, index, shift, sibs)                                  \
	{                                                                      \
		.xa = array, .xa_index = index, .xa_shift = shift,             \
		.xa_sibs = sibs, .xa_offset = 0, .xa_pad = 0,                  \
		.xa_node = XAS_RESTART, .xa_alloc = NULL, .xa_update = NULL    \
	}
...
#define XA_STATE_ORDER(name, array, index, order)                              \
	struct xa_state name =                                                 \
		__XA_STATE(array, (index >> order) << order,                   \
