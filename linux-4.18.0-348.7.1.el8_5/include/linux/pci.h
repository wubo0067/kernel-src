...
struct pci_dev {
	...
	struct resource resource
		[DEVICE_COUNT_RESOURCE]; /* I/O and memory regions + expansion ROMs */
	...
	unsigned int
		broken_parity_status : 1; /* Generates false positive parity */
	...
	RH_KABI_FILL_HOLE(
		unsigned int
			no_vf_scan : 1) /* Don't scan for VFs after IOV enablement */
	...
	struct bin_attribute
		*rom_attr; /* Attribute descriptor for sysfs ROM entry */
	int rom_attr_enabled; /* Display of ROM attribute enabled? */
	struct bin_attribute
		*res_attr[DEVICE_COUNT_RESOURCE]; /* sysfs file for resources */
	struct bin_attribute *res_attr_wc
		[DEVICE_COUNT_RESOURCE]; /* sysfs file for WC mapping of resources */
...
#define PCI_REGION_FLAG_MASK                                                   \
	0x0fU /* These bits of resource flags tell us the PCI region flags */
...
struct pci_driver {
	...
	RH_KABI_DEPRECATE_FN(int, suspend_late, struct pci_dev *dev,
			     pm_message_t state)
...
#define PCI_DEVICE_DATA(vend, dev, data)                                       \
	...
	.device = PCI_DEVICE_ID_##vend##_##dev, .subvendor = PCI_ANY_ID,       \
	.subdevice = PCI_ANY_ID, 0, 0, .driver_data = (kernel_ulong_t)(data)
...
int pcie_capability_clear_and_set_word(struct pci_dev *dev, int pos, u16 clear,
				       u16 set);
int pcie_capability_clear_and_set_dword(struct pci_dev *dev, int pos, u32 clear,
					u32 set);
...
int __printf(6, 7)
	pci_request_irq(struct pci_dev *dev, unsigned int nr,
...
#define pci_bus_for_each_resource(bus, res, i)                                 \
	for (i = 0; (res = pci_bus_resource_n(bus, i)) ||                      \
		    i < PCI_BRIDGE_RESOURCE_NUM;                               \
	     i++)

int __must_check pci_bus_alloc_resource(
	struct pci_bus *bus, struct resource *res, resource_size_t size,
...
const struct pci_device_id *
	pci_hw_vendor_status(const struct pci_device_id *ids,
			     struct pci_dev *dev);
...
static inline bool pcie_aspm_enabled(struct pci_dev *pdev)
{
	return false;
}
#endif
...
static inline int pci_request_io_regions(struct pci_dev *pdev, const char *name)
{
...
static inline int pci_request_mem_regions(struct pci_dev *pdev,
					  const char *name)
{
	return pci_request_selected_regions(
...
static inline bool pci_ats_disabled(void)
{
	return true;
}
...
dev：PCI设备结构的指针
min, max最小，最大中断向量数
flags：中断类型标志PCI_IRQ_MSIX, PCI_IRQ_MSI, PCI_IRQ_LEGACY, PCI_IRQ_AFFINITY
返回值：成功时返回实际分配的中断向量数
*/
...
#define DECLARE_PCI_FIXUP_SECTION(section, name, vendor, device, class,        \
	...
	static const struct pci_fixup __PASTE(__pci_fixup_##name, __LINE__)    \
		__used __attribute__((__section__(#section),                   \
				      aligned((sizeof(void *))))) = {          \
			vendor, device, class, class_shift, hook               \
		};

#define DECLARE_PCI_FIXUP_CLASS_EARLY(vendor, device, class, class_shift,      \
				      hook)                                    \
	DECLARE_PCI_FIXUP_SECTION(.pci_fixup_early, hook, vendor, device,      \
				  class, class_shift, hook)
#define DECLARE_PCI_FIXUP_CLASS_HEADER(vendor, device, class, class_shift,     \
				       hook)                                   \
	DECLARE_PCI_FIXUP_SECTION(.pci_fixup_header, hook, vendor, device,     \
				  class, class_shift, hook)
#define DECLARE_PCI_FIXUP_CLASS_FINAL(vendor, device, class, class_shift,      \
				      hook)                                    \
	DECLARE_PCI_FIXUP_SECTION(.pci_fixup_final, hook, vendor, device,      \
				  class, class_shift, hook)
#define DECLARE_PCI_FIXUP_CLASS_ENABLE(vendor, device, class, class_shift,     \
				       hook)                                   \
	DECLARE_PCI_FIXUP_SECTION(.pci_fixup_enable, hook, vendor, device,     \
				  class, class_shift, hook)
#define DECLARE_PCI_FIXUP_CLASS_RESUME(vendor, device, class, class_shift,     \
				       hook)                                   \
	DECLARE_PCI_FIXUP_SECTION(.pci_fixup_resume, resume##hook, vendor,     \
				  device, class, class_shift, hook)
#define DECLARE_PCI_FIXUP_CLASS_RESUME_EARLY(vendor, device, class,            \
					     class_shift, hook)                \
	DECLARE_PCI_FIXUP_SECTION(.pci_fixup_resume_early, resume_early##hook, \
				  vendor, device, class, class_shift, hook)
#define DECLARE_PCI_FIXUP_CLASS_SUSPEND(vendor, device, class, class_shift,    \
					hook)                                  \
	DECLARE_PCI_FIXUP_SECTION(.pci_fixup_suspend, suspend##hook, vendor,   \
				  device, class, class_shift, hook)
#define DECLARE_PCI_FIXUP_CLASS_SUSPEND_LATE(vendor, device, class,            \
					     class_shift, hook)                \
	DECLARE_PCI_FIXUP_SECTION(.pci_fixup_suspend_late, suspend_late##hook, \
				  vendor, device, class, class_shift, hook)
...
#define DECLARE_PCI_FIXUP_RESUME(vendor, device, hook)                         \
	DECLARE_PCI_FIXUP_SECTION(.pci_fixup_resume, resume##hook, vendor,     \
				  device, PCI_ANY_ID, 0, hook)
#define DECLARE_PCI_FIXUP_RESUME_EARLY(vendor, device, hook)                   \
	DECLARE_PCI_FIXUP_SECTION(.pci_fixup_resume_early, resume_early##hook, \
				  vendor, device, PCI_ANY_ID, 0, hook)
...
static inline void pci_iov_remove_virtfn(struct pci_dev *dev, int id)
{
}
 ...
 * -ENOENT otherwise.
 */
int pci_vpd_find_info_keyword(const u8 *buf, unsigned int off, unsigned int len,
			      const char *kw);
...
void pci_msi_register_fwnode_provider(
	struct fwnode_handle *(*fn)(struct device *));
...
#define pci_WARN(pdev, condition, fmt, arg...)                                 \
	WARN(condition, "%s %s: " fmt, dev_driver_string(&(pdev)->dev),        \
	     pci_name(pdev), ##arg)

#define pci_WARN_ONCE(pdev, condition, fmt, arg...)                            \
	WARN_ONCE(condition, "%s %s: " fmt, dev_driver_string(&(pdev)->dev),   \
		  pci_name(pdev), ##arg)
