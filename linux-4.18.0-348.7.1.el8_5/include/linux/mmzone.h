...
#define get_pageblock_migratetype(page)                                  \
	get_pfnblock_flags_mask(page, page_to_pfn(page), PB_migrate_end, \
				MIGRATETYPE_MASK)

struct free_area {
	struct list_head free_list[MIGRATE_TYPES]; // 这为什么是链表数组
	/*
  实际上，每个空闲链表会进一步拆分为多个子链表，以适应不同的页面迁移类型，这是解决内存碎片整理难题的必要设计
  */
...
enum node_stat_item {
	...
	RH_KABI_BROKEN_INSERT_ENUM(WORKINGSET_NODES) WORKINGSET_REFAULT_BASE,
	RH_KABI_BROKEN_INSERT_ENUM(WORKINGSET_REFAULT_ANON =
					   WORKINGSET_REFAULT_BASE)
		RH_KABI_BROKEN_INSERT_ENUM(WORKINGSET_REFAULT_FILE)
			WORKINGSET_ACTIVATE_BASE,
	RH_KABI_BROKEN_INSERT_ENUM(WORKINGSET_ACTIVATE_ANON =
					   WORKINGSET_ACTIVATE_BASE)
	...
	RH_KABI_BROKEN_INSERT_ENUM(NR_FILE_THPS)
		RH_KABI_BROKEN_INSERT_ENUM(NR_FILE_PMDMAPPED) NR_ANON_THPS,
	RH_KABI_BROKEN_REMOVE_ENUM(NR_UNSTABLE_NFS) NR_VMSCAN_WRITE,
...
struct lruvec {
   ...
   */
	RH_KABI_REPLACE_SPLIT(struct zone_reclaim_stat reclaim_stat,
			      unsigned long anon_cost, unsigned long file_cost,
			      /* Refaults at the time of last reclaim cycle */
			      unsigned long refaults[ANON_AND_FILE])

	/* Non-resident age, driven by LRU movement */
	atomic_long_t RH_KABI_RENAME(inactive_age, nonresident_age);
	/* Various lruvec state flags (enum lruvec_flags) */
	RH_KABI_REPLACE(unsigned long refaults, unsigned long flags)
#ifdef CONFIG_MEMCG
...
struct zone {
   ...
   */
	RH_KABI_REPLACE(unsigned long managed_pages,
...
typedef struct pglist_data {
   ...
   */
	...
	int kswapd_order;
	enum zone_type RH_KABI_RENAME(kswapd_classzone_idx,
	...
	enum zone_type RH_KABI_RENAME(kcompactd_classzone_idx,
...
#else
#define pgdat_page_nr(pgdat, pagenr) \
...
#else
static inline void memory_present(int nid, unsigned long start,
				  unsigned long end)
{
}
#endif
...
extern int numa_zonelist_order_handler(struct ctl_table *, int, void __user *,
				       size_t *, loff_t *);
...
struct mem_section {
   ...
   */
	...
	RH_KABI_REPLACE(unsigned long *pageblock_flags,
 ...
 */
bool memmap_valid_within(unsigned long pfn, struct page *page,
			 struct zone *zone);
#else
static inline bool memmap_valid_within(unsigned long pfn, struct page *page,
				       struct zone *zone)
{
	return true;
}
