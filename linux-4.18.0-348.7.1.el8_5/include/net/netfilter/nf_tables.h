...
static inline void nft_set_pktinfo(struct nft_pktinfo *pkt, struct sk_buff *skb,
...
{
	return type == NFT_DATA_VERDICT ?
		       NFT_REG_VERDICT :
			     NFT_REG_1 * NFT_REG_SIZE / NFT_REG32_SIZE;
}
...
struct nft_set_iter {
	...
	unsigned int skip;
	int err;
	int (*fn)(const struct nft_ctx *ctx, struct nft_set *set,
		  const struct nft_set_iter *iter, struct nft_set_elem *elem);
};
...
struct nft_set_ops {
	...
	void (*activate)(const struct net *net, const struct nft_set *set,
			 const struct nft_set_elem *elem);
	void *(*deactivate)(const struct net *net, const struct nft_set *set,
			    const struct nft_set_elem *elem);
	bool (*flush)(const struct net *net, const struct nft_set *set,
		      void *priv);
	void (*remove)(const struct net *net, const struct nft_set *set,
		       const struct nft_set_elem *elem);
	void (*walk)(const struct nft_ctx *ctx, struct nft_set *set,
		     struct nft_set_iter *iter);
	void *(*get)(const struct net *net, const struct nft_set *set,
...
static inline struct nft_data *
	nft_set_ext_key_end(const struct nft_set_ext *ext)
{
...
static inline struct nft_userdata *
	nft_set_ext_userdata(const struct nft_set_ext *ext)
{
...
#define NFT_SET_GC_BATCH_SIZE                                                  \
	((PAGE_SIZE - sizeof(struct nft_set_gc_batch_head)) / sizeof(void *))
...
static inline struct nft_set_gc_batch *
	nft_set_gc_batch_check(const struct nft_set *set,
			       struct nft_set_gc_batch *gcb, gfp_t gfp)
{
	if (gcb != NULL) {
...
#define NFT_EXPR_SIZE(size)                                                    \
	(sizeof(struct nft_expr) + ALIGN(size, __alignof__(struct nft_expr)))
...
#define nft_rule_for_each_expr(expr, last, rule)                               \
	for ((expr) = nft_expr_first(rule), (last) = nft_expr_last(rule);      \
	     (expr) != (last); (expr) = nft_expr_next(expr))
...
// 通过chain成员获取nft_base_chain
static inline struct nft_base_chain *
	nft_base_chain(const struct nft_chain *chain)
{
...
struct nft_object_type {
	const struct nft_object_ops *(*select_ops)(
		const struct nft_ctx *, const struct nlattr *const tb[]);
...
struct nft_object_ops {
	void (*eval)(struct nft_object *obj, struct nft_regs *regs,
		     const struct nft_pktinfo *pkt);
	unsigned int size;
	int (*init)(const struct nft_ctx *ctx, const struct nlattr *const tb[],
	...
	void (*update)(struct nft_object *obj, struct nft_object *newobj);
...
#define MODULE_ALIAS_NFT_OBJ(type) MODULE_ALIAS("nft-obj-" __stringify(type))
...
#define nft_clear(__net, __obj) (__obj)->genmask &= ~nft_genmask_next(__net)
...
#define nft_trans_rule(trans) (((struct nft_trans_rule *)trans->data)->rule)
...
#define nft_trans_set(trans) (((struct nft_trans_set *)trans->data)->set)
#define nft_trans_set_id(trans) (((struct nft_trans_set *)trans->data)->set_id)
...
#define nft_trans_elem_set(trans) (((struct nft_trans_elem *)trans->data)->set)
#define nft_trans_elem(trans) (((struct nft_trans_elem *)trans->data)->elem)
...
#define nft_trans_obj(trans) (((struct nft_trans_obj *)trans->data)->obj)
...
#else
static inline int nft_request_module(struct net *net, const char *fmt, ...)
{
	return -ENOENT;
}
#endif
