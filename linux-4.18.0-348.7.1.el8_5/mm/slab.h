...
slab_flags_t kmem_cache_flags(unsigned int object_size, slab_flags_t flags,
			      const char *name);
#else
static inline struct kmem_cache *__kmem_cache_alias(const char *name,
						    unsigned int size,
						    unsigned int align,
						    slab_flags_t flags,
						    void (*ctor)(void *))
...
#define SLAB_CORE_FLAGS                                                        \
	(SLAB_HWCACHE_ALIGN | SLAB_CACHE_DMA | SLAB_CACHE_DMA32 | SLAB_PANIC | \
...
#define SLAB_CACHE_FLAGS                                                       \
	(SLAB_MEM_SPREAD | SLAB_NOLEAKTRACE | SLAB_RECLAIM_ACCOUNT |           \
...
{
	return (s->flags & SLAB_RECLAIM_ACCOUNT) ? NR_SLAB_RECLAIMABLE_B :
...
static inline void memcg_slab_free_hook(struct kmem_cache *s_orig, void **p,
					int objects)
{
	struct kmem_cache *s;
...
static inline void memcg_slab_free_hook(struct kmem_cache *s, void **p,
					int objects)
{
}
					...
					size_t size, void **p)
{
	...
	for (i = 0; i < size; i++) {
		...
		kmemleak_alloc_recursive(p[i], s->object_size, 1, s->flags,
					 flags);
	}
...
struct kmem_cache_node {
	...
	// 该链表中对象是部分分配的slab页，slab在内存中具体的标识就是struct page
