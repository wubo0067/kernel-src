...
static struct request *__blk_mq_alloc_request(struct blk_mq_alloc_data *data)
{
	...
	if (e) {
		 ...
		 */
		if (!op_is_flush(data->cmd_flags) && e->type->ops.limit_depth &&
	...
	if (!list_empty(list)) {
		...
		if (!needs_restart ||
			...
			blk_mq_run_hw_queue(hctx, true);
			 ...
			 (ret == BLK_STS_RESOURCE || no_budget_avail))
...
void blk_mq_flush_plug_list(struct blk_plug *plug, bool from_schedule)
{
	...
	do {
		...
		// 将pos节点之前切割为两个链表，rq_list是个新链表，用于存放切割后的前半部分链表，head是原始链表头，用于存放切割后链表的后半部分
		// pos是指定的切割的位置，该节点和其之前的节点都会移动到新的链表中
...
blk_qc_t blk_mq_make_request(struct request_queue *q, struct bio *bio)
{
	...
	// 判断是否是个普通的块设备，或者是读操作，如果是是就返回current->blk_plug
...
static struct blk_mq_hw_ctx *
	blk_mq_alloc_and_init_hctx(struct blk_mq_tag_set *set,
				   struct request_queue *q, int hctx_idx,
				   int node)
{
