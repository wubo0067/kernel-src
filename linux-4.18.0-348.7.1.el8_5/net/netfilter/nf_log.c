...
// net.netfilter.nf_log_all_netns 来设置
int sysctl_nf_log_all_netns __read_mostly;
EXPORT_SYMBOL(sysctl_nf_log_all_netns);

static struct nf_logger __rcu
	*loggers[NFPROTO_NUMPROTO][NF_LOG_TYPE_MAX] __read_mostly;
...
static int netfilter_log_sysctl_init(struct net *net)
{
	...
	if (!net_eq(net, &init_net)) {
		table = kmemdup(nf_log_sysctl_table,
				sizeof(nf_log_sysctl_table), GFP_KERNEL);
		if (!table)
			goto err_alloc;
	} else {
		for (i = NFPROTO_UNSPEC; i < NFPROTO_NUMPROTO; i++) {
			snprintf(nf_log_sysctl_fnames[i], 3, "%d", i);
	...
	net->nf.nf_log_dir_header =
		register_net_sysctl(net, "net/netfilter/nf_log", table);
