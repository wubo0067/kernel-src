...
// 这个指令要求 pool_workqueue 结构体的起始地址必须是 (1 << WORK_STRUCT_FLAG_BITS) 的倍数
...
static void wq_watchdog_timer_fn(struct timer_list *unused)
{
	...
	for_each_pool(pool, pi)
	{
		...
		if (pool->cpu >= 0) {
			unsigned long cpu_touched = READ_ONCE(
...
int __init workqueue_init_early(void)
{
	...
	/*
	__alignof__用于返回该结构的对齐要求。如果返回8，则结构体的起始地址必须是8的倍数，返回是
	16，则结构体的起始地址必须是16的倍数。
	*/
