...
static struct numa_group *deref_task_numa_group(struct task_struct *p)
{
	return rcu_dereference_check(
		p->numa_group,
		p == current || (lockdep_is_held(&task_rq(p)->lock) &&
				 !READ_ONCE(p->on_cpu)));
}
...
static void util_est_dequeue(struct cfs_rq *cfs_rq, struct task_struct *p,
			     bool task_sleep)
{
	long last_ewma_diff;
	struct util_est ue;
 ...
 */
static inline bool group_smaller_min_cpu_capacity(struct sched_group *sg,
 ...
 */
static inline bool group_smaller_max_cpu_capacity(struct sched_group *sg,
						  struct sched_group *ref)
{
				     ...
				     struct sched_group *group)
{
	...
	for_each_cpu_and (i, sched_group_span(group), env->cpus) {
		 ...
		 */
		if (env->sd->flags & SD_ASYM_CPUCAPACITY &&
		    capacity_of(env->dst_cpu) < capacity && nr_running == 1)
			continue;
