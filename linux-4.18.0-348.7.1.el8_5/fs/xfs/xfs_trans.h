...
{
	return lip->li_ops->iop_unpin == NULL && lip->li_ops->iop_push == NULL;
}
...
#define xfs_trans_agblocks_delta(tp, d)                                        \
	((tp)->t_ag_freeblks_delta += (int64_t)d)
...
static inline int xfs_trans_get_buf(struct xfs_trans *tp,
				    struct xfs_buftarg *target,
				    xfs_daddr_t blkno, int numblks, uint flags,
				    struct xfs_buf **bpp)
{
...
static inline int xfs_trans_read_buf(struct xfs_mount *mp, struct xfs_trans *tp,
				     struct xfs_buftarg *target,
				     xfs_daddr_t blkno, int numblks,
				     xfs_buf_flags_t flags,
				     struct xfs_buf **bpp,
				     const struct xfs_buf_ops *ops)
{
	DEFINE_SINGLE_BUF_MAP(map, blkno, numblks);
	return xfs_trans_read_buf_map(mp, tp, target, &map, 1, flags, bpp, ops);
}
...
static inline struct xfs_log_item *
	xfs_trans_item_relog(struct xfs_log_item *lip, struct xfs_trans *tp)
{
