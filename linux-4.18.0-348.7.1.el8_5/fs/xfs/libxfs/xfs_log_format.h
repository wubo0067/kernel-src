...
typedef struct xlog_rec_header {
	...
	表示当前日志记录的日志序列号（LSN）
	通常在日志记录开始时计算。LSN 是一个唯一标识符，用于标识日志中的每一条记录。
    它包含了循环数（cycle number）和块号（block number）。h_lsn 的计算通常在日志记录创建时完成
	计算 h_lsn 的典型步骤
	1: 获取当前循环号和块号：日志系统会维护一个当前的循环号和块号，这些值会随着日志的写入而更新。
	2: 组合循环号和块号：LSN 通常是由循环号和块号组合而成的一个 64 位值。
	3: 设置 h_lsn：在日志记录头部中设置计算出的 LSN。
	*/
	__be64 h_tail_lsn; /* lsn of 1st LR w/ buffers not committed: 8
	用于指示日志中已经提交的最旧的 LSN。在日志同步操作之前需要更新 h_tail_lsn
	尾 LSN 通常表示日志中最旧的、已经提交的 LSN。这个值需要从日志子系统中获取。
	*/
...
#define XFS_LI_TYPE_DESC                                                       \
	{ XFS_LI_EFI, "XFS_LI_EFI" }, { XFS_LI_EFD, "XFS_LI_EFD" },            \
		...
		{ XFS_LI_RUI, "XFS_LI_RUI" }, { XFS_LI_RUD, "XFS_LI_RUD" },    \
...
#define XFS_ILOG_NONCORE                                                       \
	(XFS_ILOG_DDATA | XFS_ILOG_DEXT | XFS_ILOG_DBROOT | XFS_ILOG_DEV |     \
	 XFS_ILOG_ADATA | XFS_ILOG_AEXT | XFS_ILOG_ABROOT | XFS_ILOG_DOWNER |  \
	 XFS_ILOG_AOWNER)

#define XFS_ILOG_DFORK (XFS_ILOG_DDATA | XFS_ILOG_DEXT | XFS_ILOG_DBROOT)

#define XFS_ILOG_AFORK (XFS_ILOG_ADATA | XFS_ILOG_AEXT | XFS_ILOG_ABROOT)

#define XFS_ILOG_ALL                                                           \
	(XFS_ILOG_CORE | XFS_ILOG_DDATA | XFS_ILOG_DEXT | XFS_ILOG_DBROOT |    \
...
#define XFS_RMAP_EXTENT_FLAGS                                                  \
	(XFS_RMAP_EXTENT_TYPE_MASK | XFS_RMAP_EXTENT_ATTR_FORK |               \
	 XFS_RMAP_EXTENT_BMBT_BLOCK | XFS_RMAP_EXTENT_UNWRITTEN)
...
static inline size_t xfs_rui_log_format_sizeof(unsigned int nr)
{
...
static inline size_t xfs_cui_log_format_sizeof(unsigned int nr)
{
...
#define XFS_BMAP_EXTENT_FLAGS                                                  \
	(XFS_BMAP_EXTENT_TYPE_MASK | XFS_BMAP_EXTENT_ATTR_FORK |               \
...
static inline size_t xfs_bui_log_format_sizeof(unsigned int nr)
{
...
#define XFS_ALL_QUOTA_ENFD (XFS_UQUOTA_ENFD | XFS_GQUOTA_ENFD | XFS_PQUOTA_ENFD)
#define XFS_ALL_QUOTA_CHKD (XFS_UQUOTA_CHKD | XFS_GQUOTA_CHKD | XFS_PQUOTA_CHKD)

#define XFS_MOUNT_QUOTA_ALL                                                    \
