...
out_bad_rec:
	xfs_warn(mp, "%s Freespace BTree record corruption in AG %d detected!",
		...
		int flags) /* flags, XFSA_FIXUP_... */
{
	...
	if (bno_cur->bc_nlevels == 1 && cnt_cur->bc_nlevels == 1) {
		...
		if (XFS_IS_CORRUPT(mp, bnoblock->bb_numrecs !=
					       cntblock->bb_numrecs))
			return -EFSCORRUPTED;
	}
#endif
...
STATIC int xfs_alloc_update_counters(struct xfs_trans *tp, struct xfs_buf *agbp,
				     long len)
{
 ...
 * reallocate cursors.
 */
static int xfs_alloc_cur_setup(struct xfs_alloc_arg *args,
 ...
 */
static int xfs_alloc_cur_check(struct xfs_alloc_arg *args,
			       struct xfs_alloc_cur *acur,
			       struct xfs_btree_cur *cur, int *new)
{
	int error, i;
	...
	*new = 0;
	...
	diff = xfs_alloc_compute_diff(args->agbno, args->len, args->alignment,
				      args->datatype, bnoa, lena, &bnew);
	if (bnew == NULLAGBLOCK)
		goto out;
 ...
 */
STATIC int xfs_alloc_cur_finish(struct xfs_alloc_arg *args,
				struct xfs_alloc_cur *acur)
{
 ...
 */
STATIC int xfs_alloc_cntbt_iter(struct xfs_alloc_arg *args,
				struct xfs_alloc_cur *acur)
{
		...
		int *stat) /* status: 0-freelist, 1-normal/none */
{
	...
	if (args->datatype & XFS_ALLOC_USERDATA) {
		...
		error = xfs_trans_get_buf(args->tp, args->mp->m_ddev_targp,
					  XFS_AGB_TO_DADDR(args->mp, args->agno,
							   fbno),
		...
		xfs_alloc_arg_t *args) /* allocation argument structure */
{
	 ...
	 */
	args->len =
		XFS_AGBLOCK_MIN(tend, args->agbno + args->maxlen) - args->agbno;
	xfs_alloc_fix_len(args);
 ...
 */
STATIC int xfs_alloc_walk_iter(struct xfs_alloc_arg *args,
			       struct xfs_alloc_cur *acur,
			       struct xfs_btree_cur *cur, bool increment,
 ...
 */
STATIC int xfs_alloc_ag_vextent_locality(struct xfs_alloc_arg *args,
					 struct xfs_alloc_cur *acur, int *stat)
{
...
static int xfs_alloc_ag_vextent_lastblock(struct xfs_alloc_arg *args,
					  struct xfs_alloc_cur *acur,
...
restart:
	...
	if (error == -ENOSPC) {
		error = xfs_alloc_ag_vextent_small(args, acur.cnt, &bno, &len,
						   &i);
		if (error)
			goto out;
		if (i == 0 || len == 0) {
...
restart:
	 ...
	 */
	if ((error = xfs_alloc_lookup_ge(
		     cnt_cur, 0, args->maxlen + args->alignment - 1, &i)))
		goto error0;
	 ...
	 */
	if (!i) {
		error = xfs_alloc_ag_vextent_small(args, cnt_cur, &fbno, &flen,
						   &i);
		if (error)
			goto error0;
		if (i == 0 || flen == 0) {
		...
		ASSERT(i == 1);
		busy = xfs_alloc_compute_aligned(args, fbno, flen, &rbno, &rlen,
						 &busy_gen);
	} else {
		/*
		...
		for (;;) {
			...
			busy = xfs_alloc_compute_aligned(
				args, fbno, flen, &rbno, &rlen, &busy_gen);
	...
	bno_cur = xfs_allocbt_init_cursor(args->mp, args->tp, args->agbp,
					  args->agno, XFS_BTNUM_BNO);
	if ((error = xfs_alloc_fixup_trees(cnt_cur, bno_cur, fbno, flen, rbno,
					   rlen, XFSA_FIXUP_CNT_OK)))
		goto error0;
	xfs_btree_del_cursor(cnt_cur, XFS_BTREE_NOERROR);
	xfs_btree_del_cursor(bno_cur, XFS_BTREE_NOERROR);
	...
	args->len = rlen;
	args->agbno = rbno;
	if (XFS_IS_CORRUPT(args->mp, args->agbno + args->len >
					     be32_to_cpu(agf->agf_length))) {
		error = -EFSCORRUPTED;
		goto error0;
	}
 ...
 */
STATIC int xfs_free_ag_extent(struct xfs_trans *tp, struct xfs_buf *agbp,
	...
	if (haveleft && haveright) {
		 ...
		 */
		{
			...
			if ((error = xfs_alloc_get_rec(bno_cur, &xxbno, &xxlen,
						       &i)))
				goto error0;
			if (XFS_IS_CORRUPT(mp, i != 1 || xxbno != ltbno ||
						       xxlen != ltlen)) {
				error = -EFSCORRUPTED;
				goto error0;
			}
...
{
	mp->m_ag_maxlevels = xfs_btree_compute_maxlevels(
		mp->m_alloc_mnr, (mp->m_sb.sb_agblocks + 1) / 2);
}
 ...
 */
static bool xfs_alloc_space_available(struct xfs_alloc_arg *args,
				      xfs_extlen_t min_free, int flags)
{
	...
	available = (int)(pag->pagf_freeblks + agflcount - reservation -
...
int xfs_free_agfl_block(struct xfs_trans *tp, xfs_agnumber_t agno,
			xfs_agblock_t agbno, struct xfs_buf *agbp,
 ...
 */
static void xfs_agfl_reset(struct xfs_trans *tp, struct xfs_buf *agbp,
 ...
 */
STATIC void xfs_defer_agfl_block(struct xfs_trans *tp, xfs_agnumber_t agno,
				 xfs_fsblock_t agbno,
				 struct xfs_owner_info *oinfo)
{
	struct xfs_mount *mp = tp->t_mountp;
		...
		int flags) /* XFS_ALLOC_FLAG_... */
{
	...
	if (!xfs_alloc_space_available(args, need,
				       flags | XFS_ALLOC_FLAG_CHECK))
		goto out_agbp_relse;
	...
	int fields) /* mask of fields to be logged (XFS_AGF_...) */
{
	...
	static const short offsets[] = {
		...
		offsetof(xfs_agf_t, agf_seqno), offsetof(xfs_agf_t, agf_length),
 ...
 */
int /* error */
	xfs_alloc_pagf_init(xfs_mount_t *mp, /* file system mount structure */
			    xfs_trans_t *tp, /* transaction pointer */
			    xfs_agnumber_t agno, /* allocation group number */
		...
		int btreeblk) /* block came from a AGF btree */
{
	...
	if (!agflbp && (error = xfs_alloc_read_agfl(
				mp, tp, be32_to_cpu(agf->agf_seqno), &agflbp)))
		return error;
 ...
 */
int __xfs_free_extent(struct xfs_trans *tp, xfs_fsblock_t bno, xfs_extlen_t len,
		      const struct xfs_owner_info *oinfo,
...
STATIC int xfs_alloc_query_range_helper(struct xfs_btree_cur *cur,
